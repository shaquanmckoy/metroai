...
// GET /api/admin/users  -> list users (with login stats)
...
export async function POST(req: Request) {
  try {
    ...
    if (String(err?.code) === "23505") {
      return NextResponse.json(
        { ok: false, error: "That email already exists" },
        { status: 409 }
      );
    }

    return NextResponse.json(
      { ok: false, error: "Failed to add user" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    if (rows[0].role === "admin") {
      return NextResponse.json(
        { ok: false, error: "Cannot delete admin user" },
        { status: 400 }
      );
    }
